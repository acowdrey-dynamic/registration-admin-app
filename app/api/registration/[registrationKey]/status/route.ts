...
export const POST = async (request: NextRequest, { params }: { params: Promise<{ registrationKey: string }> }) => {
    ...
    try {
        await axios.patch(`https://cccbrokerdev.allcleardandc.com/shop/${registrationKey}`, {
            status: jsonBody.status,
        })

        return Response.json({ success: true })
    } catch (error: any) {
        console.error('Error fetching registrations:', JSON.stringify(error?.response?.data || error.message))
